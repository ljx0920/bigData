    ...
    public static String doGet(String url) {
        try {
            ...
            if (statusCode == HttpStatus.SC_OK) {
                ...
                log.debug("[HttpUtils] doGet success, url:{}, response:{}", url, strResult);
                return strResult;
            } else {
    ...
    public static String doPostStr(String url, String params){
        ...
        try {
            ...
            if (state == HttpStatus.SC_OK) {
                HttpEntity responseEntity = response.getEntity();
                String strResult = EntityUtils.toString(responseEntity);
                log.info("[HttpUtils] doPostStr success, url:{}, response:{}", url, strResult);
                return strResult;
            } else {
                log.error("[HttpUtils] doPostStr error, url:{}, state:{}", url, state);
            }
        } catch (ClientProtocolException e) {
            log.error("[HttpUtils] doPostStr http protocol error, url:{}", url, e);
        } catch (IOException e) {
            log.error("[HttpUtils] doPostStr IO error, url:{}", url, e);
        } finally {
            try {
                if (response != null) {
                    response.close();
                ...
                log.error("[HttpUtils] doPostStr close client error!", e);
            }
        }
        return null;
    }
