...
public class CookieUtil {
    ...
    public static String getCookieValue(HttpServletRequest request, String cookieName,
        ...
        try {
            String decodedCookieName = URLDecoder.decode(cookieName, "UTF8");
            for (Cookie aCookieList : cookieList) {
                if (aCookieList.getName().equals(decodedCookieName)) {
                    if (aCookieList.getValue() == null
                            || "null".equals(aCookieList.getValue())) {
                        return null;
                    } else {
